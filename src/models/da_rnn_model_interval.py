...
class DualAttentionRNNModelInterval(tf.keras.Model):
    def __init__(self, conf, encoder_num_hidden, decoder_num_hidden):
        ...
        self.fc_final = Dense(units=self.conf.n_future, activation=None)
    ...
    def decoder(self, x_encoded, inputs):
        ...
        for t in range(window_size):
            ...
            y_tilde = self.fc(
                tf.concat((context, tf.expand_dims(y_prev[:, t], axis=1)), axis=1))  # => (batch_size, n_future)
